...
export async function createPTY(
  ...
  const session = await env.DB.prepare(`
    SELECT s.*, dm.role FROM sessions s
    JOIN dashboard_members dm ON s.dashboard_id = dm.dashboard_id
    WHERE s.id = ? AND dm.user_id = ? AND s.status = 'active'
  `).bind(sessionId, userId).first();

  if (!session) {
    return Response.json({ error: 'Active session not found or no access' }, { status: 404 });
  }

  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const pty = await sandbox.createPTY(session.sandbox_session_id as string);

  return Response.json({
    pty,
    wsUrl: sandbox.getPTYWebSocketUrl(session.sandbox_session_id as string, pty.id, userId),
  }, { status: 201 });
}

// List PTYs in a session
export async function listPTYs(
  ...
  if (session.status !== 'active') {
    return Response.json({ ptys: [] });
  }

  const sandbox = new SandboxClient(env.SANDBOX_URL);

  try {
    const ptys = await sandbox.listPTYs(session.sandbox_session_id as string);
    return Response.json({
      ptys: ptys.map(pty => ({
        ...pty,
        wsUrl: sandbox.getPTYWebSocketUrl(session.sandbox_session_id as string, pty.id, userId),
...
export async function deletePTY(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  await sandbox.deletePTY(session.sandbox_session_id as string, ptyId);

  return new Response(null, { status: 204 });
}

// Start agent in a session
export async function startAgent(
  ...
  const session = await env.DB.prepare(`
    SELECT s.*, dm.role FROM sessions s
    JOIN dashboard_members dm ON s.dashboard_id = dm.dashboard_id
    WHERE s.id = ? AND dm.user_id = ? AND dm.role IN ('owner', 'editor') AND s.status = 'active'
  `).bind(sessionId, userId).first();
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const agent = await sandbox.startAgent(session.sandbox_session_id as string);

  return Response.json({
    agent,
    wsUrl: sandbox.getAgentWebSocketUrl(session.sandbox_session_id as string, userId),
  }, { status: 201 });
}
...
export async function getAgent(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const agent = await sandbox.getAgent(session.sandbox_session_id as string);
...
export async function pauseAgent(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const result = await sandbox.pauseAgent(session.sandbox_session_id as string);
...
export async function resumeAgent(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const result = await sandbox.resumeAgent(session.sandbox_session_id as string);
...
export async function stopAgent(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  await sandbox.stopAgent(session.sandbox_session_id as string);

  return new Response(null, { status: 204 });
}

// File operations (proxied to sandbox)
export async function listFiles(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const files = await sandbox.listFiles(session.sandbox_session_id as string, path);
...
export async function readFile(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  const content = await sandbox.readFile(session.sandbox_session_id as string, path);
...
export async function writeFile(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  await sandbox.writeFile(session.sandbox_session_id as string, path, content);
...
export async function deleteFile(
  ...
  const sandbox = new SandboxClient(env.SANDBOX_URL);
  await sandbox.deleteFile(session.sandbox_session_id as string, path);

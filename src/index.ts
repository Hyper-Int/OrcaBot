...
async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  // GET /sessions/:id/ptys - List PTYs
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'ptys' && method === 'GET') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.listPTYs(env, segments[1], auth.user!.id);
  }

  // POST /sessions/:id/ptys - Create PTY
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'ptys' && method === 'POST') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.createPTY(env, segments[1], auth.user!.id);
  }

  // DELETE /sessions/:id/ptys/:ptyId - Delete PTY
  if (segments[0] === 'sessions' && segments.length === 4 && segments[2] === 'ptys' && method === 'DELETE') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.deletePTY(env, segments[1], segments[3], auth.user!.id);
  }

  // POST /sessions/:id/agent - Start agent
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'agent' && method === 'POST') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.startAgent(env, segments[1], auth.user!.id);
  }

  // GET /sessions/:id/agent - Get agent
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'agent' && method === 'GET') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.getAgent(env, segments[1], auth.user!.id);
  }

  // POST /sessions/:id/agent/pause - Pause agent
  if (segments[0] === 'sessions' && segments.length === 4 && segments[2] === 'agent' && segments[3] === 'pause' && method === 'POST') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.pauseAgent(env, segments[1], auth.user!.id);
  }

  // POST /sessions/:id/agent/resume - Resume agent
  if (segments[0] === 'sessions' && segments.length === 4 && segments[2] === 'agent' && segments[3] === 'resume' && method === 'POST') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.resumeAgent(env, segments[1], auth.user!.id);
  }

  // POST /sessions/:id/agent/stop - Stop agent
  if (segments[0] === 'sessions' && segments.length === 4 && segments[2] === 'agent' && segments[3] === 'stop' && method === 'POST') {
    const authError = requireAuth(auth);
    if (authError) return authError;
    return sessions.stopAgent(env, segments[1], auth.user!.id);
  }
  ...
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'files' && method === 'GET') {
    ...
    return sessions.listFiles(env, segments[1], filePath, auth.user!.id);
  }

  // GET /sessions/:id/file - Read file
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'file' && method === 'GET') {
    ...
    return sessions.readFile(env, segments[1], filePath, auth.user!.id);
  }

  // PUT /sessions/:id/file - Write file
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'file' && method === 'PUT') {
    ...
    return sessions.writeFile(env, segments[1], filePath, content, auth.user!.id);
  }

  // DELETE /sessions/:id/file - Delete file
  if (segments[0] === 'sessions' && segments.length === 3 && segments[2] === 'file' && method === 'DELETE') {
    ...
    return sessions.deleteFile(env, segments[1], filePath, auth.user!.id);
  }

    ...
    <div className="space-y-4">
        ...
        {policy.canRead && (
          <div className="pl-4 space-y-2">
            ...
            >
              ...
              <option value="allowlist">Only these senders</option>
              <option value="blocklist">Block these senders</option>
            </select>
            {policy.senderFilter?.mode !== "all" && (
              <>
                ...
                />
                <DomainListInput
                  label={policy.senderFilter?.mode === "allowlist" ? "Allowed addresses" : "Blocked addresses"}
                  domains={policy.senderFilter?.addresses || []}
                  onChange={(addresses) =>
                    update("senderFilter", { ...policy.senderFilter!, addresses })
                  }
      ...
      <Section title="Sending">
        ...
        {policy.canSend && (
          <div className="pl-4 space-y-2">
            <DomainListInput
              label="Allowed recipient addresses (empty = all)"
              domains={policy.sendPolicy?.allowedRecipients || []}
              onChange={(allowedRecipients) =>
                update("sendPolicy", { ...policy.sendPolicy, allowedRecipients })
              }
              placeholder="user@example.com"
            />
            <DomainListInput
...
// Outlook Calendar Policy Editor
const OutlookCalendarPolicyEditor: React.FC<{
  policy: OutlookCalendarPolicy;
  onChange: (policy: OutlookCalendarPolicy) => void;
}> = ({ policy, onChange }) => {
  const update = <K extends keyof OutlookCalendarPolicy>(key: K, value: OutlookCalendarPolicy[K]) => {
    onChange({ ...policy, [key]: value });
  };
  ...
  return (
    <div className="space-y-4">
      <Section title="Reading">
        ...
        {policy.canRead && (
          <div className="pl-4 space-y-2">
            <label className="text-[10px] text-[var(--foreground-muted)]">Calendar filter</label>
            <select
              className="w-full text-xs rounded border border-[var(--border)] bg-[var(--background)] px-2 py-1"
              value={policy.calendarFilter?.mode || "all"}
              onChange={(e) =>
                update("calendarFilter", {
                  ...policy.calendarFilter,
                  mode: e.target.value as "all" | "allowlist",
                  calendarIds: policy.calendarFilter?.calendarIds || [],
                })
              }
            >
              <option value="all">All calendars</option>
              <option value="allowlist">Only these calendars</option>
            </select>
            {policy.calendarFilter?.mode === "allowlist" && (
              <DomainListInput
                label="Allowed calendar IDs"
                domains={policy.calendarFilter?.calendarIds || []}
                onChange={(calendarIds) =>
                  update("calendarFilter", { ...policy.calendarFilter, mode: "allowlist", calendarIds })
                }
  ...
  const renderEditor = () => {
    switch (integration.provider) {
      ...
      case "outlook_calendar":
        return (
          <OutlookCalendarPolicyEditor
            policy={policy as OutlookCalendarPolicy}
            onChange={(p) => setPolicy(p)}
          />
        );
      case "slack":

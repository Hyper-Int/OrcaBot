...
// The control plane draws ballots from its own copy of the configuration list,
// and it has to match the table: a ballot offering a clip the page no longer
// shows is one the server then refuses, and a row missing from the pool can
// never be rated. That list was being regenerated by hand after every import,
// which is exactly the sort of step that gets skipped - so the import does it.
const POOL_FILE = "../controlplane/src/tts/handler.ts";
if (fs.existsSync(POOL_FILE)) {
  const ids = rows.filter((r) => r.sample).map((r) => r.config);
  const lines = [];
  let line = "  ";
  for (const id of ids) {
    const add = `'${id}', `;
    if ((line + add).length > 78) { lines.push(line.trimEnd()); line = "  "; }
    line += add;
  }
  lines.push(line.trimEnd());
  const block = `const CONFIGS = [\n${lines.join("\n")}\n] as const;`;
  const before = fs.readFileSync(POOL_FILE, "utf8");
  const after = before.replace(/const CONFIGS = \[[\s\S]*?\] as const;/, block);
  if (after === before) {
    console.log("  = ballot pool already matches");
  } else if (!/const CONFIGS = \[/.test(before)) {
    console.warn("  ! could not find CONFIGS in the control plane - ballot pool NOT updated");
  } else {
    fs.writeFileSync(POOL_FILE, after);
    console.log(`  ~ ballot pool synced to ${ids.length} configurations`);
  }
}

...
// Deploy script for gemini.orcabot.com frozen deployment.
// Temporarily swaps .env.production with gemini values so Next.js
...
const GEMINI_ENV = `NEXT_PUBLIC_API_URL=https://api.gemini.orcabot.com
NEXT_PUBLIC_SITE_URL=https://gemini.orcabot.com
`;
...
function restoreAll() {
  // Remove the temporary gemini .env.production we wrote
...
try {
  ...
  // Swap .env.production with gemini values
  moveOut(".env.production");
  writeFileSync(path.join(cwd, ".env.production"), GEMINI_ENV);

  exitCode = run("npm", ["run", "workers:build"]);
  if (exitCode !== 0) throw new Error(`workers:build failed with exit code ${exitCode}`);

  exitCode = run("npx", ["wrangler", "deploy", "-c", "wrangler.gemini.toml"]);

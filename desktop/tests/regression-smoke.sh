...
# does. Empty when the running stack predates surface enforcement — an empty header
# is harmless there (enforcement off => dev-auth trusted headerless).
...
# Dev-auth headers for every control-plane call (user id + surface token).
...
DID="$(curl -s -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"name":"regression-smoke"}' "$CP/dashboards" \
...
sess_field() { # $1=itemId $2=field
  curl -s "${AUTH[@]}" "$CP/dashboards/$DID" | jq_py "import json,sys;d=json.load(sys.stdin);print(next((s.get('$2','') for s in d['sessions'] if s['itemId']=='$1'),'GONE'))"
}
...
b1="$(X 'pgrep -c bash')"
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$ITEM"; sleep 3
b2="$(X 'pgrep -c bash')"; z="$(X 'ps -eo stat,comm | grep -c "^Z.*bash"')"  # PTY (bash) zombies only; the browser block leaves chromium <defunct> helpers that are not a PTY-reap leak
...
IT="$(curl -s -X POST "${AUTH[@]}" -H 'Content-Type: application/json' \
...
for i in 1 2 3; do curl -s -o /dev/null -X POST "${AUTH[@]}" -H 'Content-Type: application/json' -d '{}' "$CP/dashboards/$DID/items/$IT/session" & done; wait
sleep 3
n="$(curl -s "${AUTH[@]}" "$CP/dashboards/$DID" | jq_py "import json,sys;print(sum(1 for s in json.load(sys.stdin)['sessions'] if s['itemId']=='$IT'))")"
[ "$n" = "1" ] && ok "exactly one session ($n)" || bad "duplicate sessions ($n)"
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$IT"

# ---- #3/#5 item input validation ------------------------------------------
echo "[#3/#5] item input validation"
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw 'not-json{' "$CP/dashboards/$DID/items")"
[ "$c" = "400" ] && ok "invalid JSON -> 400" || bad "invalid JSON -> $c (want 400)"
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"content":"x"}' "$CP/dashboards/$DID/items")"
[ "$c" = "400" ] && ok "missing type -> 400" || bad "missing type -> $c (want 400)"
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"type":"bogus"}' "$CP/dashboards/$DID/items")"
[ "$c" = "400" ] && ok "invalid type -> 400" || bad "invalid type -> $c (want 400)"
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"type":"note","content":"ok"}' "$CP/dashboards/$DID/items")"
...
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw 'not-json{' "$CP/dashboards/$DID/terminals/$PTY4/integrations")"
{ [ "$c" -ge 400 ] && [ "$c" -lt 500 ]; } && ok "malformed body -> $c (4xx)" || bad "malformed body -> $c (want 4xx)"
c="$(code -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"provider":"bogus","userIntegrationId":"fake"}' "$CP/dashboards/$DID/terminals/$PTY4/integrations")"
...
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$ITEM6"
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$ITEM4"

# ---- #7 dashboard delete reaps PTYs ---------------------------------------
echo "[#7] dashboard delete reaps its PTYs"
ND="$(curl -s -X POST "${AUTH[@]}" -H 'Content-Type: application/json' --data-raw '{"name":"regression-smoke-del"}' "$CP/dashboards" \
  | jq_py "import json,sys;print(json.load(sys.stdin)['dashboard']['id'])")"
d0="$(X 'pgrep -c bash')"; "$ORCABOT" new terminal shell --dash "$ND" >/dev/null 2>&1; sleep 3
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$ND"; sleep 3
d2="$(X 'pgrep -c bash')"; z="$(X 'ps -eo stat,comm | grep -c "^Z.*bash"')"  # PTY (bash) zombies only; the browser block leaves chromium <defunct> helpers that are not a PTY-reap leak
...
SSID="$(curl -s "${AUTH[@]}" "$CP/dashboards/$DID" | jq_py "import json,sys;d=json.load(sys.stdin);print(next(s['sandboxSessionId'] for s in d['sessions'] if s['itemId']=='$ITEM8'))")"
...
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$ITEM8"
curl -s -o /dev/null -X DELETE "${AUTH[@]}" "$CP/dashboards/$DID/items/$ITEM9"
